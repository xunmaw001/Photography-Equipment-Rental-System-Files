	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">租赁反馈列表</div>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
            				...
            				var fankuineirongCell  = document.createElement('td');
												  ...
												  //如果图文详情包含图片
				  if(item.fankuineirong && item.fankuineirong.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.fankuineirong.indexOf('<img>');
					var endIndex = item.fankuineirong.indexOf('>');				
					fankuineirongCell.innerHTML = item.fankuineirong.substring(beginIndex, endIndex+1);				
				  }else{
				  	 if(item.fankuineirong != null && item.fankuineirong != "" && item.fankuineirong.length >= 11){
				  		fankuineirongCell.innerHTML = item.fankuineirong.substring(0, 10) + "..."; 
				  	}else{
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("zulinfankui/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'zulinfankui/page';
		...
		function datePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																										...
																										$(function() { 
						$('#fankuiriqi_start').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
					$(function() { 
						$('#fankuiriqi_end').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
		...
		function sh(){
			...
			httpJson("zulinfankui/update","POST",data,(res)=>{
				if(res.code == 0){
					$("#closeDialog").click();
					alert("提交成功");
			...
			window.sessionStorage.setItem('crossTableName',"zulinfankui");
