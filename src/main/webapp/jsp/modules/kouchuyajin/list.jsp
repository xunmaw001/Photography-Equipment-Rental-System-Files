  ...
  <body>
	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">扣除押金列表</div>
						  ...
						  <form class="navbar-form navbar-left" id="searchForm" name="searchForm" role="search" style="margin: 3px 0 3px 0; white-space: nowrap;" onsubmit="saveSearchForm()">
							  <div class="form-group" id="form-item-group">
																					...
																					<input name="xingming" placeholder="姓名">
																			                                 																			<label>押金</label>
																					<input name="yajin" placeholder="押金">
																			                                 									                                 																			<label>日期</label>
																					<div id="riqi_start" class="input-append" style="display: inline-block;">
												<input data-format="yyyy-MM-dd" name="riqi_start" type="text"  class="form-control dtpicker" style="max-width: 95px;">
												<span class="add-on">
												<i data-time-icon="icon-time" data-date-icon="icon-calendar" class="btn btn-info btn-lg"></i>
												</span>
											</div>

											<div id="riqi_end" class="input-append" style="display: inline-block;">
												<input data-format="yyyy-MM-dd" name="riqi_end" type="text"  class="form-control dtpicker" style="max-width: 95px;">
												<span class="add-on">
												<i data-time-icon="icon-time" data-date-icon="icon-calendar" class="btn btn-info btn-lg"></i>
												</span>
											</div>
								...
								<th><input type="checkbox" onclick="chooseAll()"/></th>
																											<th onclick="sort('yonghuming')">用户名<i id="yonghumingIcon" class="fa fa-sort"></i></th>
																																				...
																																				<th onclick="sort('yajin')">押金<i id="yajinIcon" class="fa fa-sort"></i></th>
																																															<th style="min-width: 100px;">理由</th>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
        ...
        function getDataList() {
			http("kouchuyajin/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																															xingming : searchForm.xingming,	
																																	yajin : searchForm.yajin,	
																																										riqi_start : searchForm.riqi_start,
							riqi_end : searchForm.riqi_end,
																		},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
            				...
            				var liyouCell  = document.createElement('td');

												  if(item.liyou !=null && item.liyou != "" && item.liyou.length >= 11){
				  	liyouCell.innerHTML = item.liyou.substring(0, 10) + "..."; 
				  }else{
				  	liyouCell.innerHTML = item.liyou;
				  }

				
									row.appendChild(liyouCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("kouchuyajin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'kouchuyajin/page';
			...
			pageIndex = 1;
						...
						if(xingming != null && xingming.trim() != ""){
							searchForm.xingming = "%" + xingming + "%"
																						...
																						var yajin = document.forms["searchForm"]["yajin"].value;
						if(yajin != null && yajin.trim() != ""){
							searchForm.yajin = "%" + yajin + "%"
						}
									 
																													var riqi_start = document.forms["searchForm"]["riqi_start"].value;
						var riqi_end = document.forms["searchForm"]["riqi_end"].value;
						if(riqi_start != null && riqi_start != ""){
							searchForm.riqi_start = riqi_start
						}
						if(riqi_end != null && riqi_end != ""){
							searchForm.riqi_end = riqi_end
						}
		...
		function getSum(colName){
			...
			window.sessionStorage.setItem('crossTableName',"kouchuyajin");
