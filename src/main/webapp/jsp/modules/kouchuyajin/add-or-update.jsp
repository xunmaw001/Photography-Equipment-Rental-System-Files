  ...
  <body>
						...
						<form class="form-horizontal" id="addOrUpdateForm" method="post">
							<input id="updateId" name="id" type="hidden">
										...
										<div class="col-lg-6">
											...
											<div class="form-group">
										<label class="control-label col-lg-3">理由</label>
										<div class="col-lg-6">
											<textarea id="liyou" name="liyou" class="form-control" rows="3" placeholder="理由"></textarea>
										</div>
									</div>
											<div class="form-group">
	...
	<script type="text/javascript">
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					http("kouchuyajin/info/" + id,"GET",{},(res)=>{
						if(res.code == 0){
		...
		function submit() {
				...
				httpJson("kouchuyajin/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
																																															//如果跨表更新失败，不显示成功提示
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#yonghuming").val(ruleForm.yonghuming);	
																...
																$("#liyou-input").val(ruleForm.liyou);
					...
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												...
												if(res.data != null && res.data != '' && res.data.yajin != null && res.data.yajin != ''){
															...
															yajinKucun = res.data.yajin;
								$("#yajin").val(res.data.yajin);

													}
												if(res.data != null && res.data != '' && res.data.liyou != null && res.data.liyou != ''){

															$("#liyou-input").val(res.data.liyou);
													}
		...
		function crossTableUpdate(){
			...
			if(flag){
				httpJson(crossTableName + "/update","POST",{
					id: crossTableId,
																																								yajin: crossRuleForm.yajin - $('#yajin').val(),	
																																					},(res)=>{
					if(res.code == 0){
						console.log('跨表更新成功');
						return true;
		...
		function crossTableUpdateValidation(){
			//防止减法导致库存负值
																										if(yajinKucun < $('#yajin').val()){
						alert('押金不能大于'+yajinKucun+'！请重新输入');
						return false;
					}
																								return true;
		}
