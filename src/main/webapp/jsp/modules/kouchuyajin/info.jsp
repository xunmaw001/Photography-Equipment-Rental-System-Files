						...
						<form class="form-horizontal" id="addOrUpdateForm" method="post">
							<input type="hidden" name="id" v-model="ruleForm.id">
											...
											<div class="col-lg-6">
												...
												<script id="liyouEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var liyouUe = UE.getEditor('liyouEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" id="liyou-input">
											</div>
										</div>
																																																															<!-- 表单按钮 -->
							<div class="form-group">
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "kouchuyajin/info/" + id,
			...
			methods: { }
				...
				// 填充富文本框
		  		function setContent(){
				  									  									  									  											if(ruleForm.liyou != null && ruleForm.liyou != 'null' && ruleForm.liyou != ''){
							var liyouMes = '' + ruleForm.liyou;
							var liyouUeditor = UE.getEditor('liyouEditor');
							liyouUeditor.ready(function() {
								liyouUeditor.setContent(liyouMes);
								liyouUeditor.setDisabled('fullscreen');
							});
						}
									  									  				}  
