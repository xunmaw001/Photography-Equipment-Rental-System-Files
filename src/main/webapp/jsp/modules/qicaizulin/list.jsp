	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">器材租赁列表</div>
						  ...
						  <form class="navbar-form navbar-left" id="searchForm" name="searchForm" role="search" style="margin: 3px 0 3px 0; white-space: nowrap;" onsubmit="saveSearchForm()">
							  <div class="form-group" id="form-item-group">
																					...
																					<input name="qicaimingcheng" placeholder="器材名称">
																			                                 																			<label>器材类型</label>
																					<input name="qicaileixing" placeholder="器材类型">
																			                                 																			<label>每天价格</label>
																					<input name="meitianjiage" placeholder="每天价格">
																			                                 									                                 									                                 									                                 									                                 									                                 									                                 									                                 									                                 									                                 							  </div>
						  </form>
	                        ...
	                        <tr>
								<th><input type="checkbox" onclick="chooseAll()"/></th>
																											<th onclick="sort('dingdanbianhao')">订单编号<i id="dingdanbianhaoIcon" class="fa fa-sort"></i></th>
																																				...
																																				<th onclick="sort('qicaileixing')">器材类型<i id="qicaileixingIcon" class="fa fa-sort"></i></th>
																																				<th onclick="sort('meitianjiage')">每天价格<i id="meitianjiageIcon" class="fa fa-sort"></i></th>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
			...
			http("qicaizulin/page","GET",{
				...
				order: sortOrder,
																						dingdanbianhao : searchForm.dingdanbianhao,	
																																	...
																																	qicaileixing : searchForm.qicaileixing,	
																																	meitianjiage : searchForm.meitianjiage,	
																																																																																																			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
            				...
            				var qicaileixingCell  = document.createElement('td');

												 qicaileixingCell.innerHTML = item.qicaileixing;

                
									row.appendChild(qicaileixingCell);
				
            				var meitianjiageCell  = document.createElement('td');

												 meitianjiageCell.innerHTML = item.meitianjiage;

                
									row.appendChild(meitianjiageCell);
					...
					payBtn.innerHTML = "支付";           
					payBtn.setAttribute('class','btn btn-xs btn-success')
			...
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"guihaitixing\")");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("qicaizulin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'qicaizulin/page';
			...
			pageIndex = 1;
						...
						if(qicaimingcheng != null && qicaimingcheng.trim() != ""){
							searchForm.qicaimingcheng = "%" + qicaimingcheng + "%"
																						...
																						var qicaileixing = document.forms["searchForm"]["qicaileixing"].value;
						if(qicaileixing != null && qicaileixing.trim() != ""){
							searchForm.qicaileixing = "%" + qicaileixing + "%"
						}
									 
																						var meitianjiage = document.forms["searchForm"]["meitianjiage"].value;
						if(meitianjiage != null && meitianjiage.trim() != ""){
							searchForm.meitianjiage = "%" + meitianjiage + "%"
						}
		...
		function datePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																																																																				...
																																																																				$(function() { 
						$('#zulinriqi_start').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
					$(function() { 
						$('#zulinriqi_end').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
		...
		function sh(){
			...
			httpJson("qicaizulin/update","POST",data,(res)=>{
				if(res.code == 0){
					$("#closeDialog").click();
					alert("提交成功");
			...
			window.sessionStorage.setItem('crossTableName',"qicaizulin");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"qicaizulin");
